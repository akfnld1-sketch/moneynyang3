  ...
  nudge.innerHTML = `
    ...
    <div style="flex:1;min-width:0;">
      ...
      <div style="font-size:11px;color:var(--text3);line-height:1.5;margin-top:2px;">
        ${isIOS ? 'Safari 공유 버튼 → 홈 화면에 추가를 눌러주세요.' : '설치하면 홈 화면에서 앱처럼 바로 열 수 있어요.'}
      </div>
    </div>
    <button onclick="installPWA()"
      style="padding:8px 10px;border-radius:8px;border:none;background:var(--accent);color:#fff;font-size:12px;font-weight:800;font-family:'Noto Sans KR';cursor:pointer;">설치</button>
    <button onclick="hidePwaInstallNudge()" aria-label="닫기"
      style="width:28px;height:28px;border-radius:8px;border:1px solid var(--border);background:var(--surface2);color:var(--text2);font-size:14px;cursor:pointer;">×</button>`;
